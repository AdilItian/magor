...
const TranscriptRenderer = (props) => {
    ...
    let defaultTranscript = _.find(
        transcripts,
        (tx) => tx._id === props.defaultTranscript
    )
    if (!defaultTranscript && transcripts.length) {
        defaultTranscript = transcripts[0]
    }
    if (transcriptId) {
        defaultTranscript = transcripts.find((f) => f._id === transcriptId)
    }
